...
void SystemStateObject::connectToSystem(SystemObject *sys)
{
    ...
    m_startState    ->assignProperty(sys,"state",ConstValue::SystemState::Start);
    m_workingState  ->assignProperty(sys,"state",ConstValue::SystemState::Working);
    m_closeState    ->assignProperty(sys,"state",ConstValue::SystemState::Close);
...
void SystemStateObject::stateSwitch(ConstValue::SystemState state)
{
    //主动进行状态转换
    //检查
    assert(
...
#ifdef QDEBUGCHECK
    qDebug()<<"SystemStateObject::stateSwitch | connectSystem:"<<m_connectSystem->objectName()<<" stateSwitch:"<<m_connectSystem->state();
#endif
}

...
#include <memory>
#include "../CheckTool/CheckMemberTool.hpp"
/*
...
REGISTER_FUNCTION_FOR_CHECK(is_init)

REGISTER_FUNCTION_FOR_CHECK(get_id)
...
struct ConcreteCommand : Command {
    ...
    int get_id() const override {
        if constexpr (CHECK_FUNCTION_EXIST(T, get_id, int)) {
    ...
    bool is_init() const override {
        if constexpr (CHECK_FUNCTION_EXIST(T, is_init, bool)) {

...
#define FUNCTION_SOLVE_ADD__(Function) _Function
#define FUNCTION_SOLVE_REMOVE__(Function) _Function

#define REGISTER_FUNCTION_FOR_CHECK(Function) template<typename T, typename R, typename... Args>\
struct has_##Function {\
private:template<typename U, typename = decltype(std::declval<U>()._##Function(std::declval<Args>()...))>\
    static constexpr std::true_type check(T*);\
    template<typename>static constexpr std::false_type check(...);\
    template<typename U> static constexpr auto check_return_type(U*) -> typename std::is_same<decltype(std::declval<U>()._##Function(std::declval<Args>()...)), R>::type;\
...
has_##Function(Params&&... params){};\
};

#define CHECK_FUNCTION_EXIST(ObjClass, Function, RetClass, ...)                       \
    std::apply( [](auto&&...args)constexpr{                                                   \
        using Class_Function = has_##Function<std::decay_t<ObjClass>, RetClass,decltype(args)...>; \
...
private: \
template<typename C,typename =decltype(std::declval<C>().m_##Property)>\
static constexpr std::true_type check(Class*);\
template<typename>static constexpr std::false_type check(...);\
template<typename C> \
static constexpr auto check_type(C*) ->typename std::is_same<decltype(std::declval<C>().m_##Property),T>::type;\
public:\
...
has_##Property<<std::decay_t<ObjClass>,PropertyClass>::value

#define CHECK_PROPERTY_EXIST_ONLY_NAME(ObjClass,Property) \
has_##Property<<std::decay_t<ObjClass>,remove_member_pointer_t<decltype(&ObjClass::Property)> >::value
